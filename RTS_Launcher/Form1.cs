...
namespace RTS_Launcher
{
    public partial class Form1 : Form
    {
        ZipFile zip_file;
        ZipEntry game_config;
        ...
        {
            InitializeComponent();
            this.FormClosing += RTS_Launcher_FormClosing1;
        }

        /* Launcher Load */
        private void RTS_Launcher_Load(object sender, EventArgs e)
        {
            ...
            zip_file = ZipFile.Read("game.dat");
            game_config = zip_file["CONFIGS/game_core.json"];
            ...
            string[] current_resolution = new String[2];
            foreach (JProperty res_config in game_config_json["DEFAULT"]["resolution"])
            {
                if (res_config.Name == "width")
                {
                    current_resolution[0] = res_config.Value.ToString();
                }
                else if (res_config.Name == "height")
                {
                    current_resolution[1] = res_config.Value.ToString();
                }
            }
        ...
        /* Launcher Close */
        private void RTS_Launcher_FormClosing1(object sender, FormClosingEventArgs e)
        {
            ...
            foreach (JProperty keybind in game_config_json["DEFAULT"]["keybinds"])
            {
                ...
                {
                    if (labels[i].Text == keybind.Name)
                    {
                        keybind.Value = keynameToKeycode(dropdowns[i].Items[dropdowns[i].SelectedIndex].ToString());
                        break;
                    }
                }
            }
            ...
            foreach (JProperty res_config in game_config_json["DEFAULT"]["resolution"])
            {
                if (res_config.Name == "width")
                {
                    res_config.Value = Convert.ToInt32(new_resolution[0]);
                }
                else if (res_config.Name == "height")
                {
            ...
            try
            {
                ...
                if (launch_game)
                {
                    Process.Start("GameClient.exe", "Launcher_Auth");
                }
            }
            catch
            {
